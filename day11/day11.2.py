...
a = load_input()
compass = 'NESW'
x, y, facing, points = 0, 0, 'N', dict()
c = Computer(a, inp=[])

points[(x, y)] = 1  # Initial cell is White
while True:
    c.append_input(points.get((x, y), 0))
    color = next(c.compute())
    if color is None:
        break
    turn = next(c.compute())
    points[(x, y)] = color
...
# Keep only white colored cells in tracked points
points = [k for k, v in points.items() if v != 0]

# We need a hold of the canvas bounds before we start
# plotting the letters
min_i, max_i, min_j, max_j = None, None, None, None
for i, j in points:
    min_i = min(min_i, i) if min_i else i
    max_i = max(max_i, i) if max_i else i
    min_j = min(min_j, j) if min_j else j
    max_j = max(max_j, j) if max_j else j
